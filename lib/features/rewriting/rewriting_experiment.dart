...
import 'rewriting_controller.dart';

enum RewritingPresentation { technical, everyday }

class RewritingExperiment extends StatelessWidget {
  const RewritingExperiment({
    super.key,
  ...
  final RewritingController controller;
  final RewritingPresentation presentation;
  ...
  Widget build(BuildContext context) {
    switch (presentation) {
      case RewritingPresentation.technical:
        return _buildTechnical(context);
      case RewritingPresentation.everyday:
    ...
    return Column(
      ...
      children: [
        ...
        FilledButton.icon(
          ...
          label: Text(
            controller.isChecking ? 'Checking…' : 'Check rewriting status',
          ),
        ),
        ...
        FilledButton.icon(
          ...
          label: Text(
            controller.isRunning ? 'Rewriting…' : 'Rewrite professionally',
          ),
        ),
        const SizedBox(height: 16),
        Card(
      ...
      children: [
        ...
        FilledButton.icon(
          ...
          icon: controller.isRunning
              ...
              : const Icon(Icons.play_arrow),
          label: Text(
            controller.isRunning ? 'Rewriting…' : 'Run rewriting test',
          ),
        ),
        const SizedBox(height: 12),
  ...
  Color _statusColor(ColorScheme colors) {
    switch (controller.status) {
      ...
      case 'DOWNLOADING':
        return presentation == RewritingPresentation.everyday
            ? colors.primary
            : Colors.blue;
      case 'CHECKING':
        return presentation == RewritingPresentation.everyday
